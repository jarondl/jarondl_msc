...
\end{align}
(Side note to me: $\mathcal{L}[\lambda^{\eta-1}] = \Gamma(\eta)w^{-\eta}$, and some $\Gamma$ function identities lead to this result. Also: $\sinc=\frac{\sin(\pi x)}{\pi x}$)
...
%%%% 
For $1d$, the situation is more clear. While $\epsilon > 0.5$, the eigenvalue distribution matches diffusion, with changing diffusion coefficients, while for $0<\epsilon<0.5$, sub diffusion occurs, exactly as expected from the well defined theory :
