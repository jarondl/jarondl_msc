...
\begin{align}\label{eq:f_w}
 &\qquad f_w(w)dw = \left|\ln \frac{w}{w_0}\right|^{d-1} e^{-\left|\frac{\Omega_d}{d}\frac{\xi}{r_0}\ln \frac{w}{w_0}\right|^d} \Omega_d\left(\frac{\xi}{r_0}\right)^d\frac{dw}{w}
\end{align}
...
\begin{align}
\textrm{if } &\overline{(w^{-1})}\equiv \int f(w) \frac{dw}{w}< \infty \\
\textrm{then } & D = \left(\overline{(w^{-1})}\right)^{-1}
\end{align}
 It is a known property of harmonic means, that because of their inverse dependence on values, the values closer to zero contribute most of the sum. Therefore, we are interested in the distribution of transition rates in the limit $w\rightarrow 0$. This limit in equation \eqref{eq:f_w} for $1d$ is:
\begin{align}
\lim_{w\rightarrow 0} f(w)dw \sim \frac{dw}{w_0} \\
\int f(w)\frac{dw}{w} \sim \int\frac{dw}{w_0w} \rightarrow \infty 
\end{align}
Which means that the harmonic mean diverges, and the system is not diffusive. 
