...
Using the scaling hypothesis one can show that
\begin{align}
    \mathcal{P}(t)\sqrt{S(t)}=1 \\
    \mathcal{P}(t) = \frac{1}{\sqrt{S(t)}} \\
    g(\lambda) = \mathcal{L}^{-1}[\mathcal{P}(t)] = \mathcal{L}^{-1}\left[\frac{1}{\sqrt{S(t)}}\right]
\end{align}
Where $\mathcal{L}^{-1}$ is the inverse laplace transform, (The Bromwich integral).

If the diffusion can be described as $S(t)\approx 2Dt$, then:
\begin{align}
    g(\lambda) = \mathcal{L}^{-1}\left[\frac{1}{\sqrt{2Dt}}\right] = \frac{1}{\sqrt{2\pi D\lambda}} \\
    \int_0^\lambda g(\lambda')d\lambda' = \sqrt{\frac{2\lambda}{\pi D}}
\end{align}
