...
We wish to study a banded model similar to the one presented in \cite{kottos_heat},
...
All those plane waves have $PN= \frac{2}{3}N$, and we can also compute the mode velocity:
\begin{align}
v \ \ &=\ \ \frac{d\lambda_k}{dk} \ \ =\ \ -\sum_{n=1..b} 2\cdot n\cdot \sin(n\cdot k)
\end{align}
Note that this expression is not single valued. Meaning that each $lambda$ 
has more than one associated velocity. 


The $DOS$ is defined by summing the inverse velocities of each mode. 
...
&= \ \ -1 + \frac{\sin((b+\dfrac{1}{2})k)}{\sin(\dfrac{1}{2}k)}
\end{align}  
Where in the last step we have used Lagrange's trigonometric identities.
The velocity is now:
\begin{align}
v \ \ &=\ \ \frac{d\lambda_k}{dk} \ \ =\ \ -\sum_{n=1..b} 2\cdot n\cdot \sin(n\cdot k)
\end{align}
