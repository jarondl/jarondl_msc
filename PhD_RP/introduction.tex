...
%
A more general class is the detailed balance matrices (\ref{detailed_balance}), which have specific 
...
% 
\rmrk{TODO active should be added here}.
%
If the matrices have real (positive) eigenvalues, then in the long time 
limit the system will undergo relaxation to the aforementioned
states. If, on the other hand, some eigenvalues are complex, then 
the system will exhibit oscillations. There is recent interest and activity \rmrk{TODO cite some of them}
in the field of $\mathcal{P}\mathcal{T}$-symmetric matrices, which are matrices
that do not obey the parity symmetry $\mathcal{P}$ ($\hat{x}\rightarrow -\hat{x} \quad \hat{p}\rightarrow -\hat{p}$) nor
the Time reflection symmetry $\mathcal{T}$ ($\hat{x}\rightarrow \hat{x} \quad \hat{p}\rightarrow -\hat{p}\quad i\rightarrow -i$)
but for some parameters do obey the combined symmetry $\mathcal{P}\mathcal{T}$. 
The manifestation of this symmetry is having real eigenvalues, while 
the symmetry breaking is having pairs of complex conjugate eigenvalues. 
