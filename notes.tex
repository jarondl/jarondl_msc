...
\section{Spacing statistics}
 ...
 {\bf I have decided to try out another convention.} Instead of defining $w_0$ to be the rate between sites at distance $r_0$, we define $w_0$ to be the rate between sites at distance $0$. This is less numerically stable at low $\epsilon$s, but gives better insight on $\epsilon$s close to one.
...
The new convention is (on trial):
\begin{align}
...
\begin{align}
    C(r(w)) &= \exp\left[{-\frac{\Omega_d}{d}\left(-\epsilon\ln \left(\frac{w}{w_0}\right)\right)^d}\right] \\
&= \begin{cases} 
    \left(\frac{w}{w_0}\right)^{\epsilon} &d=1 \\
e^{-\pi(-\epsilon\ln \frac{w}{w_0})^2} &d=2 \\
e^{-\frac{4}{3}\pi(-\epsilon\ln \frac{w}{w_0})^3} &d=3 \\
   \end{cases}
\end{align}
The highest possible rate is $r=0\rightarrow w=w_0$, which means that $w < w_0$.
...
\begin{align}
  ...
  C(\lambda) = \sinc\left(\frac{\epsilon}{1+\epsilon}\right)\lambda^{\frac{\epsilon}{1+\epsilon}}
\end{align}
...
\caption{$1d$ n.n. network, the dotted lines represent the theoretical distributions}
\end{figure}
...
\begin{align}   W_{nm} = w_0 e^{(-r_nm)/(r_0\epsilon)} \end{align}  
...
\caption{The filled dots are the eigenvalues of a $2d$-scatter system. The empty circles are of an ordered lattice with the same all-to-all transitions. The crosses are the theoretical values for a bloch n.n. model. The dashed lines are based on the theoretical average Diffusion coefficient according to \eqref{eq:LRT_D}. The solid lines are computed by numeric summing.}
\end{figure}
\begin{figure}[H]
...
The last plot is of a bloch model, with randomized values for the 4 nn jumps. 
\begin{figure}[H]
\includegraphics[clip,width=0.9\hsize]{bloch_2d_4nn_rand_log_eig}
\end{figure}

All the $2d$ plots have a linear fit for the low eigenvalues. The fit function is $\log(y) = \log(x) + a$. The following plot is of the diffusion constant found by $D = \frac{1}{2\pi e^a}$ as a function of $\epsilon$.
...
\begin{align} 
  ...
  D/r_0^2 &= \pi \int_0^\infty w_0 e^{-\frac{r}{r_0\epsilon}} \left(\frac{r}{r_0}\right)^3 \frac{dr}{r_0} \\
  D/r_0^2 &= \pi w_0\int_0^\infty  e^{-x} (x\epsilon)^3 \epsilon dx\\
  D/r_0^2 &= 6\pi w_0 \epsilon^4 \label{eq:LRT_D}
\end{align}
\section{VRH}
...
\begin{align}  p(r)dr = w(r)n(r)dr = w_0 e^{-\frac{r}{r_0\epsilon}}2\pi \frac{r}{r_0}\frac{dr}{r_0}=2\pi w_0 e^{-\frac{r}{r_0\epsilon} + \log\frac{r}{r_0}}\frac{dr}{r_0}\end{align}  
