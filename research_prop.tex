...
\begin{align}
  S(t) \;=\; \sum_n \left(\frac{r_n(t)}{r_0(t)}\right)^2 p_n  %-\overline{r}(t)
\end{align}
...
\begin{align}
\mathcal{P}(t) &\;=\; \left(2\pi S(t)\right)^{-d/2}
\end{align}
For diffusive systems, 
\begin{align}
S(t) &\;=\; 2Dt  \\
\mathcal{P}(t) &\;=\; \left(2\pi S(t)\right)^{-d/2} \;=\; \left(4\pi D t\right)^{-d/2}
\end{align}
We can combine this result with \autoref{eq:p_t_spectrum} to obtain a relation between $g(\lambda)$ and D:
\begin{align}
    g(\lambda) &\;=\; \mathcal{L}^{-1}[\mathcal{P}(t)] \;=\; \mathcal{L}^{-1}\left[ \left(4\pi D t\right)^{-d/2}\right] \;=\; \frac{\lambda^{\frac{d}{2}-1}}{\Gamma\left(\frac{d}{2}\right)\left(4\pi D\right)^{d/2}}  \\
    C(\lambda) &\;=\; \int_{\infty}^{\lambda} g(\lambda')d\lambda' \;=\; \frac{\lambda^{\frac{d}{2}}}{\frac{d}{2}\Gamma\left(\frac{d}{2}\right)\left(4\pi D\right)^{d/2}}
\label{eq:C_D}
\end{align}
