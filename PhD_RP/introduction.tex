...
Our focus is on real-valued matrices. This ensures that the eigenvalues
are either real or  pairs of complex conjugates.
% This reminds us of
%Hamiltonians that conserve PT (Parity-Time) reversal symmetry. Actually,
%by change of basis, these Hamiltonians can probably be presented as real valued matrices. 
%
%
%These real matrices span several categories:
We define several kinds of matrices by their off-diagonal elements:
\begin{enumerate}
  \item
       \label{symmetric} Symmetric.
...
\end{enumerate}
%
The symmetric matrices (\ref{symmetric}) have $w_{nm}=w_{mn}$. 
This means that they have the very useful analogy to a simple resistor network. 
The detailed balance matrices (\ref{detailed_balance}) satisfy
the ratio
%
\begin{align}
  w_{nm} = \exp\left({-\frac{E_n-E_m}{T}}\right)
\end{align}
%
This allows the system to reach Boltzmann's equilibrium at long times.
The symmetric matrix formally obey detailed balance with $T\rightarrow\infty$.
The assymetric matrices (\ref{assymetric}) are those that do not obey detailed balance.
This kind of system can lead to interesting effects such as Sinai's diffusion.
The active networks (\ref{active}) have both positive and negative
off diagonal elements. They are called so because they can 
not be realized using only passive elements such as resistors or capacitors only.
