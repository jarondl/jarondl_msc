...
\begin{align}
  ...
  &= \int_0^{r^*} w^* r^2 2\pi rdr  + \int_{r^*}^\infty w_0e^{-\frac{r}{s}} r^2 2\pi rdr \\
  &=  \frac{\pi}{2}w_0 e^{-\frac{r^*}{s}}{r^*}^4 + \pi w_0  e^{-\frac{r^*}{s}}\left( \left(\frac{r^*}{s}\right)^3 + 3\left(\frac{r^*}{s}\right)^2 + 6\left(\frac{r^*}{s}\right) +6 \right) s^4 \\
  &=  e^{-\frac{r^*}{s}}\pi w_0\left[\frac{1}{2}{r^*}^4 +  2{r^*}^3 s + 6{r^*}^2 s^2 + 12r^*s^3 +12 s^4  \right]
\end{align}
%
When $r^* \rightarrow 0$, the solution converges to the known LRT solution: \rmrk{After removing the dubious $\frac{1}{2}$ from the LRT equation}
\beq
  D_{\tbox{ERH}}[\bm{w}] = 12\pi w_0 s^4 
\eeq
...
\eeq
%
For diffusive systems, meaning that the spreading is linear in time, we follow the usual definition of a diffusion coefficient, i.e.
%
\beq
  D \quad = \quad \lim_{t\rightarrow \infty} \frac{<x^2(t)>}{2t}
\eeq
%
Using scaling, we may write:
%
\beq
  \mathcal{P}(t) = \left(\frac{1}{2\pi d S(t)}\right)^{d/2}
\eeq
%
Whereby we obtain:
%
\beq
  \mathcal{P}(t) = \frac{d}{\Omega_d}{<r^2(t)>}^\frac{-d}{2} 
\eeq
%
