...
In our notation we can write it as:
...
\begin{align*}
    |f_w(w)dw| = |f_r(r)dr| \\
    f_w(w) = f_r(r)|\frac{dw}{dr}| = f_r(-\xi \ln w)\frac{w}{\xi} = e^{-nV_d (-\xi\ln w)^d} ndV_d (-\xi\ln w)^{d-1}
\end{align*}
...
\begin{align} 
    ...
    &\mathrm{(b) : }\qquad P(w)\rightarrow 0 \qquad\mathrm{as}\qquad w \rightarrow 0 \\
    &\mathrm{(c) : }\qquad P(w) = 
        \begin{cases} 
