...
If the system is non-negative  such as in rate equations, with $\lambda \in [0,\lambda_c]$, this ergodic mode 
sits on the lower edge of the spectrum, meaning either that the localization 
...
meaning the the ground state is localized, and the states \emph{above} the mobility edge are de-localized.



\begin{figure}
\begin{tikzpicture}[x=\textwidth/20,y=0.2cm]
\draw[black] (-1,10) --(1,10);
\draw[black] (-1.2,9) --(1.2,9);
\draw[black] (-1.5,8) --(1.5,8);
\draw[black] (-1.8,7) --(1.8,7);
\draw[black,dashed] (-3,6) rectangle (3,2);
\draw[black] (-1.8,1) --(1.8,1);
\draw[black] (-1.5,0) --(1.5,0);
\draw[black] (-1.2,-1) --(1.2,-1);
\draw[black] (-1,-2) --(1,-2);
\draw[dotted] (-4,-3) -- (+4,-3) node [pos=0.5, below] {$\lambda=-\lambda_c$};
\draw[->] (-4,-4) --(-4,11) node [pos=0.5,left] {$\lambda$};

% right side
\draw[black] (10-1,10) --(10+1,10);
\draw[black] (10-1.2,9) --(10+1.2,9);
\draw[black] (10-1.5,8) --(10+1.5,8);
\draw[black] (10-1.8,7) --(10+1.8,7);

\draw[black,dashed] (10-3,6) rectangle (10+3,2) ;
\fill[white]  (6,4) rectangle (14,0);
\draw[dotted] (10-4,4) -- (10+4,4) node [pos=0.5, below] {$\lambda=0$};
\draw[->] (10-4,-4) --(10-4,11) node [pos=0.5,left] {$\lambda$};
\end{tikzpicture}
\caption{The mobility edge. The full lines represent localized modes,
  the dashed line the mobility edge, and the dotted line the ground state.
  On the left, for a banded model, the ground state
  is localized. On the right,
  for a non-negative model, the ground state is within the de-localized area. }
     \label{fig:bands}
\end{figure}
